...
-- specific physics setup
    ...
    for i = -radius, radius do
        for j = -radius, radius do
            ...
            local i = (math.sin(d * 5.0) + 1.0) * _cube_size * h * 10.0
            i = i + 3.0
            if math.fmod((i + j + radius) * i, 2) <= 1 then
                if math.fmod((i + j + radius) * i, 5) <= 1 then
            ...
            if d < radius * _cube_size then
                local _new_node, _rb = CreatePhysicCubeEx(_scene, hg.Vec3(_cube_size, _cube_size, _cube_size), hg.TranslationMat4(hg.Vec3(x, y + i, z)), _cube_ref, {_generic_material}, hg.RBT_Dynamic, 1)
